...
function collectTranslatableBlocks(root: ParentNode): Array<{ element: Element; text: string }> {
  ...
  for (const el of elements) {
    ...
    if (
      (tagLower === 'div' || tagLower === 'section' || tagLower === 'article') &&
...
async function markPairReady(
  ...
  try {
    const storageNs: 'session' | 'local' = chrome.storage.session ? 'session' : 'local'
...
async function getOrCreateTranslator(
  ...
  const translator = await api.create({
    ...
    monitor(m: EventTarget) {
      ...
      m.addEventListener('downloadprogress', (e: TranslatorDownloadProgressEvent) => {
...
async function getOrCreateLanguageDetector(
  ...
  const cached = window[cacheKey] as LanguageDetectorInstance | undefined
  if (cached) return cached
  let lastPct = -1
  const detector = await api.create({
    monitor(m: EventTarget) {
      if (!onProgress) return
      m.addEventListener('downloadprogress', (e: LanguageDetectorDownloadProgressEvent) => {
        const loaded = e.loaded || 0
        const pct = Math.round(loaded * 100)
        if (pct !== lastPct) {
          lastPct = pct
...
async function detectSourceLanguageForPage(
  onProgress?: (pct: number) => void,
): Promise<{ lang: LanguageCode; confidence: number } | null> {
  const api = window.LanguageDetector
  if (!api) return null
  try {
